...
impl VerletIntegration {
    ...
    #[allow(dead_code)]
    pub fn verlet_integration(&mut self) {
        for i in 0..self.vec1.points.len() {
            self.vec1.points[i].x = 2.0 * self.vec2.points[i].x - self.vec3.points[i].x + self.dt * self.dt * self.gravitation(i).x;
            self.vec1.points[i].y = 2.0 * self.vec2.points[i].y - self.vec3.points[i].y + self.dt * self.dt * self.gravitation(i).y;
        }
        for i in 0..self.vec1.points.len() {
    ...
    #[allow(dead_code)]
    pub fn gravitation(&mut self, index: usize) -> kd_tree::Grid2D {
